...
public class AESFastEngine {
    ...
    private int[][] generateWorkingKey(byte[] key,
        ...
        for (int i = 0; i < key.length; t++) {
            w[t >> 2][t
                & 3] = (key[i] & 0xff) | ((key[i + 1] & 0xff) << 8) | ((key[i + 2] & 0xff) << 16)
                | (key[i + 3] << 24);
            i += 4;
        }
    ...
    private transient int rounds;
    private transient int[][] workingKey = null;
    private transient int c0, c1, c2, c3;
    private transient boolean forEncryption;
...
//  }
     ...
     * @param theKey        the parameters required to set up the cipher.
     *                                  inappropriate.
     */
    public void init(boolean forEncryption,
        byte[] theKey) {
        workingKey = generateWorkingKey(theKey, forEncryption);

        System.out.println();
        System.out.println("key=");
        for (int i = 0; i < 10; i++) {
            for (int j = 0; j < 4; j++) {
                System.out.print(Integer.toHexString(workingKey[i][j]) + " ");
            }
        }
...
//    return;
     ...
     * @param ivec byte[]
     */
    public void cbc_encrypt(byte[] in,
        byte[] out,
        int inLen,
        byte[] ivec) {
        ...
        System.arraycopy(ivec, 0, iv, 0, BLOCK_SIZE);
        int n;
        int len = inLen;
        int off = 0;
        while (len >= BLOCK_SIZE) {
            for (n = 0; n < BLOCK_SIZE; ++n) {
                tmp[n] = (byte) (in[n + off] ^ iv[n]);
            }
            processBlock(tmp, 0, out, off);
            System.arraycopy(out, off, iv, 0, BLOCK_SIZE);
            len -= BLOCK_SIZE;
            off += BLOCK_SIZE;
        }
        if (len > 0) {
            ...
            processBlock(tmp, 0, tmpOut, 0);
            System.arraycopy(tmpOut, 0, out, off, BLOCK_SIZE);
            System.arraycopy(tmp, 0, iv, 0, BLOCK_SIZE);
        }
    }
     ...
     * @param ivec byte[]
     */
    public void cbc_decrypt(byte[] in,
        byte[] out,
        int inLen,
        byte[] ivec) {
        byte[] tmp = new byte[BLOCK_SIZE];
        ...
        if (len > 0) {
            ...
            processBlock(tmp, 0, tmpOut, 0);
            for (n = 0; n < len; ++n) {
                out[n + off] = (byte) (tmpOut[n] ^ iv[n]);
            }
            System.arraycopy(tmpOut, 0, iv, 0, BLOCK_SIZE);
        }
    }
    ...
    private void packBlock(byte[] bytes,
        int off) {
        int index = off;
