            ...
            return this.each(function() {
                ...
                elem_table.off("click", "thead th").on("click", "thead th", function() {
                    ...
                    if(th_field) {
						...
						*/
                        /*Here modified by Quan for qualites's requirement  2015.2.12 */
                        switch(current_order) {
                        case "ascending":
                            new_order = "descending";
                            break;
...
//                    columns: s.columns,	//注释掉列头，不作为参数提交，因为影响自定义的操作列的方法调用  edit by Gongzhong 2014/12/25
                    sorting: s.sorting,
                ...
                success: function(data) {
                    ...
                    for(i in s.columns) {
                        if(column_is_visible(s.columns[i])) {
                            ...
                            if(s.columns[i].width) {
                            	tbl_html += '<th class="' + s.commonThClass + '" width="'+s.columns[i].width+'px">' + s.columns[i].header + sortingIndicator + '</th>';
                            } else {
                    ...
                    for(row in page_data) {
                        ...
                        for(i in s.columns) {
                        	// edit by Gongzhong 2014/12/25：自定义了操作列
                        	if (s.columns[i].renderer && typeof s.columns[i].renderer == 'function') {
                        		tbl_html += '<td>' + s.columns[i].renderer(page_data[row]) + '</td>';
                        	} else {
                        		if(column_is_visible(s.columns[i])) {
